...
/// Response the authenticator completes the challenge. Then send the response and the public key for the RP to verify.
...
func CreateAttestationResponse(rp RelyingParty, auth Authenticator, cred Credential, options AttestationOptions) string {
	...
	publicKeyData := cred.Key.AttestationData() // so the AttestationData is the public key?

	credData := []byte{}
	credData = append(credData, auth.Aaguid[:]...)
	credData = append(credData, bigEndianBytes(len(cred.ID), 2)...) // what is this actually??
	...
	sig, err := cred.Key.Sign(digest) // signing using the private key
	if err != nil {
